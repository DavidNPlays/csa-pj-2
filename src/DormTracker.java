...
public class DormTracker {
    public static void main(String[] args) {
        ...
        people.add(new Roommate("Jenny", 7, "Chips", "Quiet solo", 204));
        ...
        String[] chores = {"Vacuum", "Trash", "Dishes", "Laundry"};
        ...
        while (running) {
            ...
            // TODO: Ask for time and show people who wake up earlier
                ...
                int wakeTime = Integer.parseInt(scanner.nextLine());
                for (int i = 0; i < people.size(); i++)
                {
                    if (wakeTime>people.get(i).getWakeUpTime())
                    {
            ...
            // TODO: Assign chores to each day of the week and display them
                ...
                {
                    System.out.println("Day " + i + ": " + chores[i % chores.length]);
                }
            ...
            // Hint : One or both Person objects are null
                ...
                Person second = null;
                for (int i = 0; i < people.size(); i++) {
                    if (people.get(i).name.equals(name1)) {
                        first = people.get(i);
                    }
                    if (people.get(i).name.equals(name2)) {
                        second = people.get(i);
                    }
                }
                if ((first.studyStyle.equals(second.studyStyle))&&(Math.abs(first.wakeUpTime-second.wakeUpTime)<=1))
                {
            ...
            // TODO: Prompt user to add a new RA or Roommate
                ...
                int newWakeTime = Integer.parseInt(scanner.nextLine());
                ...
                if (type.equals("Roommate"))
                {
                    ...
                    people.add(new Roommate(newName, newWakeTime, newSnack, newStudyStyle, newRoomNum));
                }
                else {
                    System.out.print("Enter floor number: ");
                    int newFloor = Integer.parseInt(scanner.nextLine());
                    people.add(new RA(newName, newWakeTime, newSnack, newStudyStyle, newFloor));
                }
